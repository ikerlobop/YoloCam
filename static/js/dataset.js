 ...
 * - container: nodo contenedor (position:relative)
 * - img: <img> cargado
 * - boxesPx: [{x,y,w,h,cls}] en píxeles
...
function drawOverlayBoxes(container, img, boxesPx, {labelNames=[], colorsByIndex=null}={}){
  ...
  boxesPx.forEach(b=>{
    ...
    node.className = 'bbox';
    node.style.left = `${bx}px`; node.style.top = `${by}px`;
    node.style.width = `${bw}px`; node.style.height = `${bh}px`;
    ...
    lbl.textContent = (labelNames[b.cls] ?? String(b.cls));
...
function updateSelectionUI(split){
  ...
  const countEl = document.getElementById(`selection-count-${split}`);
  const delBtn  = document.getElementById(`delete-${split}`);
...
function clearSelection(split){
  ...
  if (grid){
    grid.querySelectorAll('.thumb-checkbox').forEach(cb=>{
      cb.checked = false;
...
async function deleteSelected(split){
  ...
  if (!confirm(`¿Eliminar ${selected.length} imagen(es) del conjunto ${split}?`)) return;

  try{
    const res = await fetch(`/dataset/delete/${split}`,{
...
function openDsLightbox(split, index){
  currentSplit = split; currentIndex = index;
...
function closeDsLightbox(){
  ...
  currentSplit = null; currentIndex = 0;
}
...
function renderSplit(split, payload){
  ...
  (payload.items || []).forEach((it, idx)=>{
    ...
    card.querySelector('.name').onclick = () => openDsLightbox(split, idx);
    ...
    const ensureOverlay = async () => {
      ...
      if (!labels.length){ if (ov) ov.innerHTML = ''; return; }
      const iw = img.naturalWidth, ih = img.naturalHeight;
